...
const APP_VERSION = "B612-Vault v5.2";
const VERSION_UPDATED = "16/08/2026 · 18:40";
...
function wireStaticEvents() {
  ...
  document.getElementById("homeExportBtn").addEventListener("click", exportBackup);
  document.getElementById("homeImportInput").addEventListener("change", importBackup);
...
async function handleGoogleLogin() {
  try {
    setCloudMessage("A abrir login Google…");
    await signInWithGoogle();
  } catch (err) {
    setCloudMessage(`Não foi possível iniciar o login Google. ${err.message || ""}`.trim());
  }
}
...
function refreshCloudUI() {
  ...
  if (isAnonymousUser(user)) {
    ...
    stateText.textContent = "Podes usar a app assim, mas para sincronizar entre dispositivos entra com Google.";
    connectCard.classList.remove("hidden");
  } else {
    badge.className = "pill ok";
    badge.textContent = "Login feito";
    userValue.textContent = user.email || user.id;
    stateText.textContent = "Conta Google ligada. Sincronização cloud disponível.";
