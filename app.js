...
function statusLabel(status) {
  return ({collection:"Coleção",wishlist:"Wishlist",build:"Build",sold:"Vendido"})[status] || status;
}
...
function renderCards() {
  ...
  for (const item of items) {
    ...
    node.querySelector(".price-label").textContent = item.status === "wishlist" ? "Preço alvo" : item.status === "sold" ? "Último valor" : "Valor";
