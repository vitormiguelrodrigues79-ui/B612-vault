...
function injectStyles(){
  ...
  const style=document.createElement("style"); style.id="catalogViewStyles";
  style.textContent=`
  ...
  .grid.view-details .card-head{align-items:flex-start}
  .grid.view-details .card-head>div:not(.score){min-width:0}
  .grid.view-list{grid-template-columns:1fr;gap:10px}
  .grid.view-list .card{padding:14px 16px}
  .grid.view-list .card-head{align-items:center;display:grid;grid-template-columns:72px minmax(0,1fr) auto;gap:14px}
  .grid.view-list .perfume-photo{width:72px;height:72px;padding:5px;border-radius:12px}
  .grid.view-list .profile,.grid.view-list .chips,.grid.view-list .similarity-card,.grid.view-list .card-foot{display:none}
  .grid.view-list .card h3{font-size:1.25rem;margin:3px 0 5px}
  .grid.view-list .score{min-width:46px;height:46px;font-size:1rem}
  .grid.view-list .status{font-size:.68rem}
  .grid.view-thumbs{grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}
  .grid.view-thumbs .card{padding:14px}
  .grid.view-thumbs .card-head{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px}
  .grid.view-thumbs .perfume-photo{grid-column:1/-1;width:100%;height:170px;padding:12px;border-radius:16px}
  .grid.view-thumbs .brand{margin-top:4px}
  .grid.view-thumbs .card h3{font-size:1.16rem;line-height:1.1}
  .grid.view-thumbs .profile,.grid.view-thumbs .chips,.grid.view-thumbs .similarity-card,.grid.view-thumbs .card-foot{display:none}
  .grid.view-thumbs .score{min-width:42px;height:42px;font-size:.96rem}
  @media(max-width:850px){.grid.view-thumbs{grid-template-columns:repeat(3,minmax(0,1fr))}}
  @media(max-width:560px){
    .view-switcher{justify-content:stretch}.view-switcher .view-label{width:100%}.view-btn{flex:1;min-width:0;padding:9px 6px}
    .grid.view-details .perfume-photo{width:70px;height:84px}
    .grid.view-list .card-head{grid-template-columns:62px minmax(0,1fr) auto;gap:10px}.grid.view-list .perfume-photo{width:62px;height:62px}
    .grid.view-thumbs{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.grid.view-thumbs .perfume-photo{height:138px}
    .grid.view-thumbs .card{padding:10px}.grid.view-thumbs .card h3{font-size:1.02rem}.grid.view-thumbs .brand{font-size:.62rem}.grid.view-thumbs .status{font-size:.62rem}.grid.view-thumbs .score{min-width:38px;height:38px;font-size:.88rem}
  }`;
...
function ensureSwitcher(){
  ...
  const host=document.createElement("div"); host.className="view-switcher"; host.id="viewSwitcher"; host.setAttribute("aria-label","Modo de visualização");
  host.innerHTML=`<span class="view-label">Vista</span><button type="button" class="view-btn" data-view="details">Detalhe</button><button type="button" class="view-btn" data-view="list">Lista</button><button type="button" class="view-btn" data-view="thumbs">Miniatura</button>`;
...
function setView(mode){
  const allowed=["details","list","thumbs"],next=allowed.includes(mode)?mode:"details",grid=document.getElementById("grid"); if(!grid)return;
  grid.classList.remove("view-details","view-list","view-thumbs"); grid.classList.add(`view-${next}`);
...
function enhanceCard(card){
  ...
  if(!img){img=document.createElement("img");img.className="perfume-photo";img.alt=`Frasco de ${name}`;img.loading="lazy";img.decoding="async";img.referrerPolicy="no-referrer";const head=card.querySelector(".card-head");if(head)head.insertBefore(img,head.firstChild)}
  if(img.src!==src)img.src=src;
  img.onerror=()=>{img.removeAttribute("src");img.style.display="none"};
}
function enhanceCards(){document.querySelectorAll("#grid .card").forEach(enhanceCard)}
